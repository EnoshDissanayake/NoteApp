...
export function NewnoteUI({ navigation, route }) {
    ...
    function getCategoryfromdb(){
          ...
          fetch("http://192.168.1.112/vivaAppWeb/getcategory.php")
          .then((response) => {
    ...
    function sendNoteDataRequest(){
        ...
        fetch("http://192.168.1.112/vivaAppWeb/savenote.php",
      {
        method: "POST",
  ...
  const ui = (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View>
            <Text>Title</Text>
            <TextInput style={styles.TextInput} onChangeText={(newTText) => setTitle(newTText)}/>
            <Text>Desciption</Text>
            <TextInput style={styles.TextInput} onChangeText={(newDText) => setDescription(newDText)}/>
            <Text>Category</Text>
            <DropDownPicker
                open={open}
                value={value}
